...
# Get IP addresses
IPV4_CURRENT=$(curl -fsSL ipv4.icanhazip.com || printf '0.0.0.0') || echo 'Failed to contact icanhazip over IPv4, logging address as 0.0.0.0'
IPV6_CURRENT=$(curl -fsSL ipv6.icanhazip.com || printf '::') || echo 'Failed to contact icanhazip over IPv6, logging address as ::'
...
# Get cloudflare IDs
CF_ZONE_ID="$($CURL "$CF_API/zones?name=$CF_DOMAIN" | sed -e 's/[{}]/\n/g' | grep '"name":"'"$CF_DOMAIN"'"' | sed -e 's/,/\n/g' | grep '"id":"' | cut -d'"' -f4)" || echo "Zone IDs not retrieved."

CF_RECORD_ID_4="$($CURL "$CF_API/zones/$CF_ZONE_ID/dns_records?type=A&name=$CF_SUBDOMAIN.$CF_DOMAIN" | sed -e 's/[{}]/\n/g' | grep '"name":"'"$CF_SUBDOMAIN"'.'"$CF_DOMAIN"'"' | sed -e 's/,/\n/g' | grep '"id":"' | cut -d'"' -f4)" || echo "ID not retrieved for A record."
CF_RECORD_ID_6="$($CURL "$CF_API/zones/$CF_ZONE_ID/dns_records?type=AAAA&name=$CF_SUBDOMAIN.$CF_DOMAIN" | sed -e 's/[{}]/\n/g' | grep '"name":"'"$CF_SUBDOMAIN"'.'"$CF_DOMAIN"'"' | sed -e 's/,/\n/g' | grep '"id":"' | cut -d'"' -f4)" || echo "ID not retrieved for AAAA record."

# Get IP addresses on CloudFlare
CF_RECORD_IPV4="$($CURL "$CF_API/zones/$CF_ZONE_ID/dns_records/$CF_RECORD_ID_4" | sed -e 's/[{}]/\n/g' | sed -e 's/,/\n/g' | grep '"content":"' | cut -d'"' -f4)" || echo 'Current IP not retrieved from A record.'
CF_RECORD_IPV6="$($CURL "$CF_API/zones/$CF_ZONE_ID/dns_records/$CF_RECORD_ID_6" | sed -e 's/[{}]/\n/g' | sed -e 's/,/\n/g' | grep '"content":"' | cut -d'"' -f4)" || echo 'Current IP not retrieved from AAAA record.'

# Write if IPv4 inconsistent
if [ "$IPV4_CURRENT" != "$CF_RECORD_IPV4" ]; then
    $CURL -X PUT "$CF_API/zones/$CF_ZONE_ID/dns_records/$CF_RECORD_ID_4" --data '{"type":"A","name":"'"$CF_SUBDOMAIN"'","content":"'"$IPV4_CURRENT"'","proxied":false}' 1>/dev/null || echo 'Failed to rewrite A record.'
    CHANGED_IPV4=true
fi

# Write if IPv6 inconsistent
if [ "$IPV6_CURRENT" != "$CF_RECORD_IPV6" ]; then
    $CURL -X PUT "$CF_API/zones/$CF_ZONE_ID/dns_records/$CF_RECORD_ID_6" --data '{"type":"AAAA","name":"'"$CF_SUBDOMAIN"'","content":"'"$IPV6_CURRENT"'","proxied":false}' 1>/dev/null || echo 'Failed to rewrite AAAA record.'
    CHANGED_IPV6=true
fi

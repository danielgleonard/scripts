...
# Get IP addresses
IPV4_CURRENT=$(curl -fsSL ipv4.icanhazip.com || printf '0.0.0.0')
IPV6_CURRENT=$(curl -fsSL ipv6.icanhazip.com || printf '::')

# no connection
if [ $IPV4_CURRENT == "0.0.0.0" ]; then
    >&2 echo 'No connection to the Internet.'
	exit 101
fi
...
if [ "$IPV4_CURRENT" != "$CF_RECORD_IPV4" ]; then
    $CURL -X PUT "$CF_API/zones/$CF_ZONE_ID/dns_records/$CF_RECORD_ID_4" --data '{"type":"A","name":"'"$CF_SUBDOMAIN"'","content":"'"$IPV4_CURRENT"'","proxied":false}' 1>/dev/null
    CHANGED_IPV4=true
fi

# Write if IPv6 inconsistent
if [ "$IPV6_CURRENT" != "$CF_RECORD_IPV6" ]; then
    $CURL -X PUT "$CF_API/zones/$CF_ZONE_ID/dns_records/$CF_RECORD_ID_6" --data '{"type":"AAAA","name":"'"$CF_SUBDOMAIN"'","content":"'"$IPV6_CURRENT"'","proxied":false}' 1>/dev/null
    CHANGED_IPV6=true
fi
